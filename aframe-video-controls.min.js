        ...
        position_control_from_camera: function() {
            ...
            if (e) {
                var i = e.el.getAttribute("rotation"),
                    a = i.y;
                t.y_position = e.position.y - 1.3, t.x_position = -t.data.distance * Math.sin(a * Math.PI / 180), t.z_position = -t.data.distance * Math.cos(a * Math.PI / 180), t.el.setAttribute("position", [t.x_position, t.y_position, t.z_position].join(" ")), this.el.object3D.lookAt(new THREE.Vector3(e.position.x, e.position.y - 1.3, e.position.z))
            }
        },
        init: function() {
            var t = this;
            this.bar_steps = 10, this.current_step = 0, this.el.setAttribute("visible", !0), this.video_selector = this.data.src, this.video_el = document.querySelector(this.video_selector), t.play_image_src = document.getElementById("video-play-image") ? "#video-play-image" : "https://res.cloudinary.com/dxbh0pppv/image/upload/c_scale,h_512,q_10/v1471016296/play_wvmogo.png", t.pause_image_src = document.getElementById("video-pause-image") ? "#video-pause-image" : "https://res.cloudinary.com/dxbh0pppv/image/upload/c_scale,h_512,q_25/v1471016296/pause_ndega5.png", this.play_image = document.createElement("a-image"), this.video_el.paused ? this.play_image.setAttribute("src", t.play_image_src) : this.play_image.setAttribute("src", t.pause_image_src), this.video_el.addEventListener("ended", function() {
                ...
                t.position_control_from_camera(), this.addEventListener("dblclick", function() {
                   ...
                   // var label = '   Clicked at : ' + parseFloat(e.ray.origin.x).toFixed(2) +', '+parseFloat(e.ray.origin.y).toFixed(2)+', '+parseFloat(e.ray.origin.z).toFixed(2);
                    var el = document.createElement('a-entity');
                    var entity_count = (document.getElementsByTagName("a-entity").length) - 3;
                    el.setAttribute('position', {x: e.ray.origin.x, y: e.ray.origin.y, z: e.ray.origin.z+(e.ray.origin.z*0.1)})
                    //el.setAttribute('position', {x: e.ray.origin.x, y: e.ray.origin.y, z: e.ray.origin.z})
                    el.setAttribute('look-at', '#camera')
                    el.setAttribute('id', entity_count)
                    el.setAttribute('geometry', {
                      ...
                      el.setAttribute('material', {
                        ...
                        var cduraion = myTime(document.querySelector('#video_1').currentTime);
                        var duration = myTime(document.querySelector('#video_1').duration);

                        var vtime = cduraion +"/"+ duration;

                        var retrievedObject = localStorage.getItem("myStorage");
                        var olddata = JSON.parse(retrievedObject);
                        console.log("z value =",e.ray.origin.y);
                        if(olddata == null) {
                            var olddata = [];
                        } 

                        var obj = {"x": parseFloat(e.ray.origin.x).toFixed(2), "y": parseFloat(e.ray.origin.y).toFixed(2),"z": parseFloat(e.ray.origin.z).toFixed(2),"time":vtime};
                        olddata.push(obj);

                        localStorage.setItem('myStorage', JSON.stringify(olddata));

                       
                     // console.log(document.querySelector('#video_1').currentTime);
                    ...
                    document.querySelector('a-scene').appendChild(el);
                     el.addEventListener('click', event => {
                        ...
                        //console.log("click event ",touchPoint1);
                        var retrievedObject = localStorage.getItem("myStorage");
                        var getjson = JSON.parse(retrievedObject);
                        var getvalues= getjson[event.detail.intersection.object.el.id];
                        ////console.log(getjson[0].x);
                       // console.log(event.detail.intersection.object.el.id);
                        document.querySelector("#x_val").setAttribute('value', "x : "+getvalues.x);
                        document.querySelector("#y_val").setAttribute('value', "y  : "+getvalues.y);
                        document.querySelector("#z_val").setAttribute('value', "z : "+getvalues.z);
                        document.querySelector("#v_val").setAttribute('value', "Time : "+getvalues.time);
             ...
             //,
            //     t.position_control_from_camera(), this.addEventListener('click', event => {
...
function myTime(time) {
  ...
  function myFunction() {
      var btn = document.getElementById("play-button");	
        ...
        setTimeout(function(){  btn.value = 'Play'; }, 1000);
			} else {
        document.querySelector("#video_1").play();
        setTimeout(function(){  btn.value = 'Pause'; }, 1000);
